...
--emial
CREATE DOMAIN Biblioteka.email AS TEXT CONSTRAINT valid_value CHECK (VALUE ~ '.+@.+');
...
--isbn
CREATE DOMAIN Biblioteka.isbn AS TEXT CONSTRAINT valid_value CHECK (VALUE ~ '^(?:ISBN(?:-1[03])?:?\ )?(?=[0-9X]{10}$|(?=(?:[0-9]+[-\ ]){3})[-\ 0-9X]{13}$|97[89][0-9]{10}$|(?=(?:[0-9]+[-\ ]){4})[-\ 0-9]{17}$)(?:97[89][-\ ]?)?[0-9]{1,5}[-\ ]?[0-9]+[-\ ]?[0-9]+[-\ ]?[0-9X]$');
COMMENT ON DOMAIN Biblioteka.isbn IS 'Domain represent isbn-10 and isbn-13 number.';

--url
CREATE DOMAIN Biblioteka.url AS TEXT CONSTRAINT valid_value CHECK (VALUE ~ '[-a-zA-Z0-9@:%._\+~#=]{2,256}\.[a-z]{2,6}\b([-a-zA-Z0-9@:%_\+.~#?&//=]*)');
